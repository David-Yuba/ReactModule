...
function DynamicForm({fields, onSubmit}){
    ...
    fields.forEach((field) => {
        initialState[field.name] = field.defaultValue ?? (field.type == "checkbox") ? false : "";
    ...
    function validateField(field, rawValue) {
        ...
        // a) Required
        if(field.required && (value === false || value === null || value === undefined || value === "")){
        ...
        if(field.pattern && value){
            ...
            if(!regex.test(value)) 
                return field.patternMessage || `${field.label} nije u ispravnom formatu`
        }
        // c) Email
        ...
        if(field.minLength?.enabled && typeof value === "string") {
            if(value.length < field.minLength.value)
                return field.minLength.message || `${field.label} mora imati minimalnu ${field.minLength.value} znakova`
        }
        // e) Max length
        if(field.maxLength?.enabled && typeof value === "string") {
            if(value.length > field.maxLength.value)
                return field.maxLength.message || `${field.label} mora imati maksimalno ${field.maxLength.value} znakova`
        }
        // f) Range (brojevi)
        if(field.range?.enabled && value !== "" && value !== null && value !== undefined){
            const num = Number(value);
            if(!Number.isNaN(num))
                if(num < field.range.min || num > field.range.max)
                    return field.range.message || `${field.label} mora biti između ${field.range.min} i ${field.range.max}`;
        }
    ...
    function handleChange(e, field){
        const nextValue = field.type === "checkbox" ? e.target.checked : e.target.value;
            ...
            {fields.map((field) => {
                ...
                return (
                    <div key={field.name} className="form-row">
                        ...
                        {hasErrors && (
                            <p id={`${id}_error`} className="error"> {errors[field.name]} </p>
                        )}
                    </div>
                )
            })}
